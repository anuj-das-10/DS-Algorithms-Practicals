...
//Supports --> Deletion from Beginning, Given Position and End
void Delete_Node()
{	
	struct Node *temp;
    int index;
...
// Condition to Delete Node from the Beginning
    else if(index-1==0){
        temp=head;
        head= temp->link;
        temp->link=NULL;
...
// Condition to Delete Node from the Given Position and End of the List (Length() will provide the number of end node)
	else{
	int i;
...
//Defining Function Prototype Search_Element(), that will Search for the elements in the Linked List by Traversing each Node
 void Search_Element(int x)
 {  
 	int position=1;
 	struct Node *temp;
    temp=head;
    if(temp==NULL){
        printf("\nList is Empty, Can not find the Element.\n\n");
    }
    else{
        while (temp!=NULL)
        {
